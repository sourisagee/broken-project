...
class TaskController {
  static async getAllTasks(_req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json(formatResponse(500, 'Failed to get tasks', null, errorMessage));
    }
  }
  ...
  static async getTasksByUserId(req: Request, res: Response): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      res.status(500).json(formatResponse(500, 'Failed to get user tasks', null, errorMessage));
    }
  }
  ...
  static async createTask(req: Request, res: TypedResponse): Promise<void> {
    try {
      const { title, genre, description } = req.body as {
        title?: string;
        genre?: string;
        description?: string;
        user_id?: number;
      ...
      if (!genre || typeof genre !== 'string' || genre.trim().length === 0) {
        res.status(400).json(formatResponse(400, 'Genre is invalid'));
        return;
      }

      if (!description || typeof description !== 'string' || description.trim().length === 0) {
        res.status(400).json(formatResponse(400, 'Description is invalid'));
        return;
      }

      const newTask = await TaskService.createTask({ title, genre, description, user_id });

      res.status(201).json(formatResponse(201, 'Task successfully created', newTask));
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      res.status(500).json(formatResponse(500, 'Failed to create task', null, errorMessage));
    }
  }
  ...
  static async updateTask(req: Request, res: TypedResponse): Promise<void> {
    try {
      ...
      if (genre !== undefined) (task as unknown as { genre: string }).genre = genre;
      if (description !== undefined)
        (task as unknown as { description: string }).description = description;
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      res.status(500).json(formatResponse(500, 'Failed to update task', null, errorMessage));
    }
  }
  ...
  static async deleteTask(_req: Request, res: Response): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      res.status(500).json(formatResponse(500, 'Failed to delete task', null, errorMessage));
    }
  }
}
