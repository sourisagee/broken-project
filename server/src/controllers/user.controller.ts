...
class UserController {
  ...
  static async refreshTokens(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!refreshToken) {
        res.status(401).json(formatResponse(401, 'Refresh token is missing', null, 'No token'));
        return;
      }
      ...
      const { accessToken: newAccessToken, refreshToken: newRefreshToken } = generateJwtTokens({
        user: decoded.user,
      });
  ...
  static async signUp(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      res.status(500).json(formatResponse(500, 'Internal server error', null, errorMessage));
    }
  }
  ...
  static async signIn(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (!isPasswordValid) {
        res.status(400).json(formatResponse(400, 'Невалидный пароль', null, 'Невалидный пароль'));
        return;
      }
  ...
  static async updateUserById(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (email && !User.validateEmail(email)) {
        res
          .status(400)
          .json(formatResponse(400, 'Некорректный формат почты', null, 'Некорректная почта'));
        return;
      }
      ...
      if (email) {
        ...
        if (existingUser && existingUser.id !== Number(userId)) {
          res
            .status(400)
            .json(
              formatResponse(400, 'Эта почта уже используется', null, 'Эта почта уже существует'),
            );
          return;
        }
      }
